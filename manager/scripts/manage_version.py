...
def update_uv_lock_package_version(new_version: str) -> None:
    """Update only the esup-runner-manager package version line in uv.lock."""
    ...
    pattern = (
        r"(\[\[package\]\]\n" r'name\s*=\s*"esup-runner-manager"\n' r'\s*version\s*=\s*)"[^"]*"'
    )
    ...
    raise ValueError('Unable to find [[package]] entry for name = "esup-runner-manager" in uv.lock')
